...
def restore_model(hps, model, checkpoint_path):
    ...
    if checkpoint_path != '':
        ...
        # since we only need 0~depth
        checkpoint_filtered = {k: v for k, v in checkpoint['model'].items() if (not k.startswith("prior.transformer._attn_mods")
                                                                                or int(k.split(".")[3]) < depth)
                               }
        
        model.load_state_dict(checkpoint_filtered)
