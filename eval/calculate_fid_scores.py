...
from features.aist_features import extract_kinetic_features
import glob
import tqdm
...
if __name__ == "__main__":
    ...
    real_features = {
        "kinetic": [np.load(f) for f in sorted(glob.glob("./datasets/gdance/eval_features_bugged/*_kinetic.npy"))],
    }

    real_features['kinetic'] = np.concatenate(real_features['kinetic'], axis=0) # shape (total_dancers, 72)
    ...
    for result_file in tqdm.tqdm(result_files):
        ...
        sample_features = []
        for n in range(keypoints3d.shape[0]): #for each individual
            sample_features.append(extract_feature(keypoints3d[n], "kinetic"))
        sample_features = np.stack(sample_features, axis=0) # (N,72)

        result_features["kinetic"].append(sample_features)
        # break

    result_features['kinetic'] = np.concatenate(result_features['kinetic'], axis=0)  # shape (total_dancers, 72)
    ...
    print('\nEvaluation: FID_k: {:.4f}'.format(FID_k))
